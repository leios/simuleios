...
*
*   ERROR: simulate function in infinite loop, make_list must be wrong.
*
*-----------------------------------------------------------------------------*/
...
int main(void){
    ...
    for (const auto &it : list){
        std::cout << it.rtime << '\t' << count << '\t' << it.part1  
                  << '\t' << it.part2 << std::endl;
        count++;
...
std::vector<Interaction> make_list(const std::vector<Particle> &curr_data,
    ...
    double vdotr, epsilon = 0.001;

    // Step 1 -- find interactions
    for (unsigned int i = 0; i < curr_data.size(); i++){

        // checking for interactions with the wall.
        if (curr_data[i].vel_x > 0 + epsilon){
        ...
        if (curr_data[i].vel_y > 0 + epsilon){
        ...
        if (curr_data[i].vel_z > 0 + epsilon){
    ...
    std::sort(std::begin(walltime), std::end(walltime),
        ...
        for (unsigned int j = 0; j < curr_data.size(); j++){
            if (i != j){
                ...
                del_z = curr_data[i].pos_z - curr_data[j].pos_y;

                del_vx = curr_data[i].vel_x - curr_data[j].vel_y;
                ...
                // Step 2 -- update list
                if (taptime <= 0 - epsilon || taptime >= 0 + epsilon){
                ...
                // note: wall will be in the -1-6 slot
                else{
                    list[i] = walltime[0];
                }
            }
