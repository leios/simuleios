...
function signal_plot(i, signal1, signal2, out; filebase = "out",
    ...
    n2 = length(signal2)
    if n2 < n1
        signal2 = [zeros(n1-n2);signal2]
    elseif n1 > n2
        signal1 = [zeros(n2-n1);signal1]
    end
    shift_signal = real([signal2[i:n1-1];signal2[1:i]])
...
function conv_lin(signal1::Array{Complex{Float64},1},
    ...
    for i = 1:n-1
        if i+length(signal2) <= length(signal1)
            rsum = sum(signal1[i:i+length(signal2)-1].*signal2)
        else
