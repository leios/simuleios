...
@kernel function polar_muller_noreplacement!(input_pts, output_pts, sigma, mu)
    ...
    r_0 = input_pts[tid, 1]^2 + input_pts[tid, 2]^2

    # this method is only valid for points within the unit circle
    if r_0 == 0 || r_0 > 1
        output_pts[tid,1] = NaN
        output_pts[tid,2] = NaN
    else
        output_pts[tid,1] = sigma * input_pts[tid,1] *
                            sqrt(-2 * log(r_0) / r_0) + mu
        output_pts[tid,2] = sigma * input_pts[tid, 2] *
                            sqrt(-2 * log(r_0) / r_0) + mu
    end
...
@kernel function cartesian_kernel!(input_pts, output_pts, sigma, mu)
    ...
    r = sqrt(-2 * log(input_pts[tid,1]))
    theta = 2 * pi * input_pts[tid, 2]

    output_pts[tid,1] = sigma * r * cos(theta) + mu
    output_pts[tid,2] = sigma * r * sin(theta) + mu
end
